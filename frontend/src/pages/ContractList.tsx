...
import { listPageSizeOptions, usePersistentPageSize } from '@/lib/listPreferences'
...
export function ContractList() {
  ...
  const [pageSize, setPageSize] = usePersistentPageSize(user?.id, 'contracts', 10)
  ...
  const canChooseDepartment = user?.role === 'admin'
  const currentDepartmentName = user?.primary_org_name || user?.department || '本部门'
  ...
  const loadContracts = async () => {
    ...
    const result = await contractApi.list({
      ...
      department: canChooseDepartment && departmentFilter !== 'all' ? departmentFilter : undefined,
  ...
  useEffect(() => {
    loadContracts()
  }, [page, pageSize, typeFilter, departmentFilter, statusFilter, sortField, sortOrder, canChooseDepartment])
