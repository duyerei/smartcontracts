...
def list_orgs(
    ...
    orgs = db.query(OrgUnit).order_by(OrgUnit.level.asc(), OrgUnit.sort.asc(), OrgUnit.id.asc()).all()
    return {"items": [_serialize_org(item) for item in orgs]}
...
def get_org_tree(
    ...
    nodes = {}
    for org in orgs:
        nodes[org.id] = {**_serialize_org(org), "children": []}
