...
class User(Base):
    ...
    dingtalk_user_id = Column(String(100), index=True, nullable=True)
    dingtalk_union_id = Column(String(100), index=True, nullable=True)
    dingtalk_open_id = Column(String(100), index=True, nullable=True)
    dingtalk_corp_id = Column(String(100), index=True, nullable=True)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)


class DingTalkLoginCandidate(Base):
    __tablename__ = "dingtalk_login_candidates"
    ...
    employee_no = Column(String(100), nullable=True)
    department_name = Column(String(200), nullable=True)
    position_name = Column(String(100), nullable=True)
    visitor = Column(Boolean, default=False)
    status = Column(String(20), default="pending", index=True)
    bound_user_id = Column(Integer, index=True, nullable=True)
...
_SQLITE_INDEX_STATEMENTS = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_users_dingtalk_union_id ON users(dingtalk_union_id)",
    "CREATE INDEX IF NOT EXISTS idx_users_dingtalk_open_id ON users(dingtalk_open_id)",
    "CREATE INDEX IF NOT EXISTS idx_users_dingtalk_corp_id ON users(dingtalk_corp_id)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_dingtalk_candidates_user_id ON dingtalk_login_candidates(dingtalk_user_id)",
    "CREATE INDEX IF NOT EXISTS idx_dingtalk_candidates_union_id ON dingtalk_login_candidates(dingtalk_union_id)",
    "CREATE INDEX IF NOT EXISTS idx_dingtalk_candidates_open_id ON dingtalk_login_candidates(dingtalk_open_id)",
    "CREATE INDEX IF NOT EXISTS idx_dingtalk_candidates_status ON dingtalk_login_candidates(status)",
]
