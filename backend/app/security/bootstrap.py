...
def _backfill_user_orgs(db: Session, root: OrgUnit):
    other_org = ensure_department_org(db, "其他", parent=root)
    changed = False
    ...
    for user in db.query(User).all():
        ...
        target_org = None
        if user.department:
            target_org = ensure_department_org(db, user.department, parent=root)
        elif user.role == "admin":
            target_org = root
        else:
            target_org = other_org
        if target_org:
...
def bootstrap_security_data(db: Session):
    root = ensure_root_org(db)
    for department_name in _iter_legacy_departments(db):
        ensure_department_org(db, department_name, parent=root)
    ensure_department_org(db, "其他", parent=root)
